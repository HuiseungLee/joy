...
- 대중교통 경로 계산 후 도보·승차·환승·하차 순서, 노선, 정류장, 행선지, 예상 시각을 팝업으로 안내
- `Ctrl` 없이 마우스 휠로 지도 확대·축소
- 브라우저 GPU 호환성을 고려한 명시적 래스터 지도 렌더링
- 로그인 보호, JSON 내보내기, SQLite 영구 저장
- 아이디 없이 비밀번호 하나로 간편 로그인, IP별 로그인 시도 제한
- 모바일·데스크톱 반응형 화면
...
로그인 화면은 아이디 없이 비밀번호만 입력합니다. 기존 설정은 보존하고 비밀번호만 바꾸려면 NAS SSH에서 다음 명령을 실행하세요. 새 비밀번호는 화면에 표시되지 않으며, 기존 `.env`는 자동으로 백업됩니다.

```bash
cd /volume1/docker/joy-map
sh scripts/set-password.sh
```

인터넷에 공개된 주소이므로 최소 8자 이상을 사용하고, 다른 사이트에서 쓰는 비밀번호는 재사용하지 마세요. 로그인 실패는 IP별로 5분 동안 최대 10회로 제한됩니다.
...
Google Maps Platform의 공식 지원표에서는 대한민국의 자동차 길찾기를 지원하지 않습니다. 그래서 국내 장소만 선택한 자동차 경로는 카카오내비 길찾기 API를 서버에서 호출하고, 해외 자동차 경로와 대중교통은 Google Routes API를 사용합니다. 카카오 REST API 키는 브라우저로 전달되지 않습니다.

카카오 키가 아직 없으면 국내 장소 7곳까지의 **클릭 순서** 자동차 길찾기는 카카오맵 새 창으로 자동 연결됩니다. 앱의 Google 지도 위에 경로선을 직접 표시하고 순서 무관 최적화를 사용하려면 아래 REST API 키를 설정해야 합니다.
